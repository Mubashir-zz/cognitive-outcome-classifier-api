...
## Complete evidence-chain check

When downstream execution artifacts exist, run:

```bash
python training/verify_cns_expansion_readiness.py \
  --workspace /private/path/cns_v8 \
  --output /private/path/cns_v8/CNS_EXPANSION_READINESS_AUDIT.json
```

The auditor checks that all five candidates, selection, quantized artifact, one-time internal test, frozen human labels, post-freeze overlap flags, frozen challenge predictions, weighted validation, staging evidence, and release decision belong to one unbroken hash chain. It cannot open the sealed answer key and cannot deploy. `WAITING_FOR_REQUIRED_EXECUTIONS` is expected before real GPU, adjudication, and staging work is complete.
