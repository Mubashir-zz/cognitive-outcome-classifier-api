...
def main() -> None:
    ...
    scientific_checks = {
        "human_records": summary.get("adjudicated", 0) >= gates["scientific"]["required_human_records"],
        "ambiguous_rate": summary.get("ambiguous", 300) / max(1, summary.get("records", 300)) <= gates["scientific"]["maximum_ambiguous_rate"],
        ...
        "high_confidence_false_negatives": summary.get("union_high_confidence_false_negatives") is not None and summary["union_high_confidence_false_negatives"] <= gates["scientific"]["maximum_high_confidence_false_negatives"],
    }
